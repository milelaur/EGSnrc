...
typedef EGS_BaseGeometry *(*getInputsFunction)();
...
GeometryViewControl::GeometryViewControl(QWidget *parent, const char *name)
    ...
    for(const auto& lib : libraries) {
        ...
        if (createGeom) {
            ...
            EGS_InputBlock *inputBlock = geom->getInputBlock();
            ...
            if(getInputs) {
                EGS_BaseGeometry *geom = getInputs();
                if(geom) {
                    EGS_InputBlock inputBlock = geom->getInputBlock();
...
//        comboBox_geomTemplate->addItem((geometryNames[i] + " (" + g->getGeometry(geometryNames[i])->getType() + ")").c_str(), geometryNames[i].c_str());
//    }
    egsFatal("testfinal\n");
