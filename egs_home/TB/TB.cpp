...
extern __extc__ void F77_OBJ_(egs_scale_xcc, EGS_SCALE_XCC)(const int*, const EGS_Float*);
extern __extc__ void F77_OBJ_(egs_scale_bc, EGS_SCALE_BC)(const int*, const EGS_Float*);
...
int TB::initScoring()
{
    ...
    if (options)
    {
        ...
        if (!err && !err1)
        {
            ...
            else
            {
                ...
                for (int j = 0; j < regions.size(); j++)
                {
                    ...
                    if (nb > 0 && regions[j] >= 0 && regions[j] < nreg + 2)
                    {
                        int ij = regions[j];
                        if (tmp[ij])
                            egsInformation("There is already a "
            ...
            egsWarning("initScoring(): you must provide both, 'regions'"
                       " and 'bins' input\n");
        delete options;
    }
...
int TB::ausgab(int iarg)
{
    ...
    int np = the_stack->np - 1;
    if (iarg == BeforeBrems || iarg == BeforeAnnihRest || (iarg == BeforeAnnihFlight && the_stack->latch[np] > 0))
    {
